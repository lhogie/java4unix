...
public class NewVersionNotifications
{
	...
	public static void checkForNewVersion(final String project)
	{
		...
		new Thread()
		{
			...
			public void run()
			{
				try
				{
					String s = new String(NetUtilities
							.retrieveURLContent(url + "releases/last-version.txt"));
					...
					if (s.matches("[0-9.]+"))
					{
						...
						if (lastVersion.isNewerThan(getVersion(project)))
						{
							System.out.println("New version " + lastVersion
									+ " is available at " + url);
							System.out
									.println("Current version is " + getVersion(project));
						}
					}
				}
				catch (Throwable e)
	...
	public static Version getVersion(String project)
	{
		...
		v.set(new String(
				new JavaResource("/" + project + "-version.txt").getByteArray()));
		return v;
	}
}
