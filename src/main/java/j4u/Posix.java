...
public class Posix {
	public static Collection<AbstractFile> find(Directory file) {
		...
		for (String filename : TextUtilities.lines(new String(Proces.exec("find", file.getPath())))) {
	...
	public static Relation<Integer, File> findFileInodes(File searchRoot) {
		...
		for (String line : TextUtilities.lines(
				new String(Proces.exec("find", searchRoot.getAbsolutePath(), "-type", "f", "-printf", "%i %p\\n")))) {
