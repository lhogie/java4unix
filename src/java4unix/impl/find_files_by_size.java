...
public class find_files_by_size extends J4UScript
{
	...
	public int runScript(CommandLine cmdLine)
	{
		...
		List<? extends AbstractFile> fileList = recursive
				? Directory.getCurrentDirectory().retrieveTree()
		...
		for (String criterionValue : sc)
		{
			...
			{
				printWarning(
						"No file match criterion " + criterion + "=" + criterionValue);
			}
			else
			{
	...
	private String getSingleCriterionValue(AbstractFile f, String c)
	{
		...
		{
			throw new IllegalArgumentException("invalid criterion: " + c
					+ ". Allowed values are " + getCommandLineSpecification()
	...
	{
		specs.add(new OptionSpecification("--criterion", "-c", ".+", "name",
				"set the group criterion"));
		specs.add(new OptionSpecification("--recursive", "-r", null, null,
				"set the group criterion"));
