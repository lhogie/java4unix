...
public class ImageReencoder extends Java4UnixCommand
{
	...
	public ImageReencoder(RegularFile f)
	{
		super(f);
	...
	{
		v.add(new OptionSpecification("--keep", "-k", null, null,
		...
		v.add(new OptionSpecification("--target-resolution", "-r",
				"[0-9]+([km])?|same|fullHD|vga|gtab3", "same",
	...
	{
		for (String inputFilename : cmdLine.findParameters())
		{
	...
	private void processArgument(AbstractFile f, CommandLine cmdLine) throws IOException
	{
		...
		{
			for (AbstractFile c : ((Directory) f).getChildren())
			{
	...
	private void pr(RegularFile file, CommandLine cmdLine) throws IOException
	{
		...
		{
			printMessage(
					"skipping image " + file.getPath() + " because it has '.big' flag");
		}
		...
		{
			printMessage(
					"skipping image " + file.getPath() + " because it is a backup file");
		}
		else
		{
	...
	private void recodeOrScale(RegularFile inputFile, CommandLine cmdLine)
			throws IOException
	{
		...
		else
		{
			...
			{
				printMessageWithNoNewLine(
						", recoding, it has " + numberOfPixels + " pixels");
				recode(inputFile, inputImage, cmdLine);
	...
	private void recode(RegularFile inputFile, BufferedImage inputImage,
			CommandLine cmdLine) throws IOException
	{
		saveResultImageToDisk(inputFile, cmdLine, inputImage, inputImage);
	}

	private void scale(RegularFile inputFile, BufferedImage image, int newwidth,
			int newheight, CommandLine cmdLine) throws IOException
	{
		// scales
		printMessageWithNoNewLine(", downscaling");
		Image resultImage = image.getScaledInstance(newwidth, newheight,
				Image.SCALE_SMOOTH);
	...
	private void saveResultImageToDisk(RegularFile inputFile, CommandLine cmdLine,
			Image resultImage, BufferedImage image) throws IOException
	{
		BufferedImage destinationImage = Utilities.toBufferedImage(resultImage,
				image.getColorModel());

		// write to temporary file
		printMessageWithNoNewLine(", encoding");
		RegularFile outFile = RegularFile.createTempFile(inputFile.getParent(), "j4u-",
				"-tmp.jpg");
		ImageIO.write(destinationImage, "jpg", outFile.createWritingStream());

		double compressionRatio = MathsUtilities
				.round((double) inputFile.getSize() / (double) outFile.getSize(), 1);

		if (compressionRatio < 1.3)
		{
			printMessage(", compression " + compressionRatio
					+ " is not worthy. Keeping original file");
			outFile.delete();
			markProcessed(inputFile);
		}
		else
